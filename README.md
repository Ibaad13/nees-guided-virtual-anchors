...
real Monte Carlo numbers rather than assuming it works.
...
than left implicit. A full 3-D deployment would instead carry attitude in
the state itself.
...
from ordinary multipath fading.
...
not the raw corrected or predicted state — decoupling it further from the
short-timescale correlation identified above. It's assigned a weaker
covariance ($\sigma_{\mathrm{VA}}^2 > \sigma_{\mathrm{LOS}}^2$) than a real
LOS anchor, and before any update is applied, both a per-row and a
